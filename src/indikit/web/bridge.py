...
:class:`~indikit.client.IndiClient`) and relays its activity to every connected
...
modelled in :mod:`indikit.web.control_frames`) for things the INDI protocol
...
from indikit import __version__
...
#: default for ``Bridge(message_history=...)``, which ``indikit serve`` fills
#: from ``INDIKIT_MESSAGE_HISTORY``.
...
#: ``Bridge(max_backlog=...)``, which ``indikit serve`` fills from
#: ``INDIKIT_MAX_BACKLOG``.
_MAX_BACKLOG = 512
...
class Bridge:
    ...
    server : str, optional
        ...
        to :data:`indikit.__version__`. A parameter rather than a lookup at
