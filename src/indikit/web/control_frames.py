...
(``{"tag": ...}``, from :mod:`indikit.protocol.models`) or one of the frames
...
class _Frame(BaseModel):
    ...
    :class:`indikit.protocol.models._Model`: a browser built against a newer
...
class HelloFrame(_Frame):
    ...
    server : str
        The INDIkit version serving this socket, for a UI to display and for a
        bug report to quote. It has **no model default on purpose**: a default
        lands in ``model_json_schema()``, so pinning ``indikit.__version__``
...
#: reason :data:`~indikit.protocol.models.IndiMessage` is: an ``event`` this
