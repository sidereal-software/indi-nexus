...
from indikit.driver.config import (
    ConfigDocument,
    config_path,
    read_document,
    remove_document,
...
from indikit.driver.dispatch import iter_new_handlers, on_new
...
CONFIG_PERSISTED = "INDIKIT_CONFIG_PERSISTED"
...
class Device:
    ...
    def _publish_persisted(self) -> None:
        """Define ``INDIKIT_CONFIG_PERSISTED``, once :meth:`setup` has returned.
    ...
    def _refresh_persisted(self) -> None:
        """Restate ``INDIKIT_CONFIG_PERSISTED`` after the membership may have moved.
    ...
    def _config_file(self) -> Path:
        ...
        if self._config_dir is None:
            raise ConfigError(
                f"{self._device} has nowhere to keep its configuration; set INDIKIT_CONFIG_DIR"
            )
    ...
    async def _on_config_write(self, vector: SwitchVector) -> None:
        ...
        :class:`~indikit.ConfigError`. The likeliest one is not the
    ...
    def _forget(self, prop: BoundProperty[Any]) -> bool:
        ...
        withdrawn still writes it. ``INDIKIT_CONFIG_PERSISTED`` stops naming it at
    ...
    def _bind(self, emit: Emit, *, config_dir: Path | None = None) -> None:
        ...
        config_dir : Path or None, optional
            ...
            persistence methods raising :class:`~indikit.ConfigError`.
        """
        self._emit = emit
    ...
    async def _dispatch_get_properties(self, request: GetProperties) -> None:
        ...
        ``INDIKIT_CONFIG_PERSISTED`` here too, once :meth:`setup` has returned and
    ...
    async def _dispatch_new(self, vector: Vector) -> None:
        ...
        several drivers on one stream (e.g. ``indikit serve --device``) relies
