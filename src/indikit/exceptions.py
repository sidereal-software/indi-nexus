"""The exception hierarchy every INDIkit failure belongs to.
...
class IndiError(Exception):
    """Base class for every error INDIkit raises on purpose.
...
class WrongPropertyKind(IndiError, TypeError):
    ...
    :meth:`~indikit.driver.property.BoundProperty.select` on a vector kind
...
class NotConnectedError(IndiError, ConnectionError):
    ...
    Raised by every :class:`~indikit.client.IndiClient` send rather than
    queueing the message for a connection that may be an hour away, and used to
    fail the waiters still parked on
    :meth:`~indikit.client.IndiClient.wait_for` when the client is closed.
...
class ConfigError(IndiError, OSError):
    ...
    operating-system error string, both of which go to the ``indikit``
    logger instead.
    """
