"""The ``indikit`` command-line interface.
...
* ``run`` - run one or more drivers (:class:`~indikit.driver.Device`
...
``INDIKIT_*`` environment is read by :class:`~indikit.settings.Settings`
...
``INDIKIT_TOKEN``, ``INDIKIT_ALLOW_INSECURE_BIND`` and
``INDIKIT_ALLOWED_ORIGINS`` as fallbacks for its own ``WEB_*`` names, because
...
from indikit.driver import Device, serve_stdio
from indikit.logging_config import configure_logging
from indikit.settings import LogLevel, settings

app = typer.Typer(help="INDIkit - modern INDI tooling.", no_args_is_help=True)
...
def main(
    ...
    log_level: Annotated[
        ...
        typer.Option(
            ...
            help="Logging level for indikit and uvicorn. "
            "(env var: INDIKIT_LOG_LEVEL; this flag wins)",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    wire: Annotated[
        ...
        typer.Option(
            "--wire",
            help="Log one line per INDI message in each direction, on indikit.wire. "
            "BLOB payloads are reported by size and never printed. "
            "(env var: INDIKIT_WIRE_LOG; this flag wins)",
        ),
    ] = None,
) -> None:
    ...
    log_level : LogLevel or None
        The level to log at, or ``None`` to take ``INDIKIT_LOG_LEVEL``.
    verbose : bool
        Shorthand for ``--log-level DEBUG``; it wins over ``--log-level``.
    wire : bool or None
        Whether to turn the ``indikit.wire`` logger up to DEBUG, or ``None``
        to take ``INDIKIT_WIRE_LOG``.
    """
    config = settings()
...
def run_devices(devices: list[Device]) -> None:
    ...
    Deliberately **not** :func:`indikit.driver.run`. That is the other process
...
"""{device_name}: an INDI driver built on INDIkit.

To try it out, run it in the web panel from the directory holding this file::

    indikit serve --device {module_name}:{class_name}
...
from indikit.protocol import IPState, ISRule, ISState, Number, NumberVector, Switch, SwitchVector
...
def new(
    ...
    typer.echo(f"  indikit serve --device {path.stem}:{class_name}")
...
def serve(
    ...
    token: Annotated[
        ...
        typer.Option(
            "--token",
            help="Shared token required on /ws and /api. (env var: INDIKIT_TOKEN; this flag wins)",
        ),
    ] = None,
    allow_origin: Annotated[
        ...
        typer.Option(
            ...
            "(env var: INDIKIT_ALLOWED_ORIGINS, space separated; this flag wins)",
        ),
    ] = None,
    allow_insecure_bind: Annotated[
        ...
        typer.Option(
            ...
            "(env var: INDIKIT_ALLOW_INSECURE_BIND; this flag wins)",
        ),
    ] = None,
) -> None:
    ...
    options is also an INDIKIT_* environment variable; the flag wins, and its absence
    falls back to the variable. See docs/docker.md or DEVELOPMENT.md for the full list.
    """
    from indikit.web.security import is_loopback
    ...
    if not chosen_token and not is_loopback(host) and not insecure:
        raise typer.BadParameter(
            ...
            "Pass --token (or INDIKIT_TOKEN), or --allow-insecure-bind to accept that."
        )
    if device:
    ...
    uvicorn.run(
        ...
        host=host,
        port=port,
        # Without this uvicorn stays at its own default while indikit.* moved,
