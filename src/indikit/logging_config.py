...
:func:`configure_logging` is the **only** place INDIkit installs a log handler,
and it is called from entrypoints alone - the CLI callback and
:func:`indikit.driver.run`. A library import configures nothing, because a
...
driver's *stdout is the INDI wire*: :func:`indikit.driver.runtime._open_stdio`
...
**The wire logger.** ``indikit.wire`` is one name for one question - "what is
...
from indikit.protocol import BLOBVector, DefVector, IndiMessage, NewVector, SetVector

#: The one logger name for wire traffic, in both directions and on both sides.
WIRE_LOGGER = "indikit.wire"

#: The line format used when INDIkit configures logging itself. Terse on
