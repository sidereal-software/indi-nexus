...
peer of ``indiserver``) are driven by the same minimal pair of callables: an async
``read`` that returns the next chunk of bytes (``b""`` at EOF) and an async
``write`` that emits one serialised message. Keeping the aliases - and the TCP
adapter - in one place lets the two layers agree on the contract and lets tests
substitute in-memory streams for either side.
"""
...
) -> tuple[ReadFn, WriteFn]:
    """Open a TCP connection and adapt it to ``read``/``write`` callables.
