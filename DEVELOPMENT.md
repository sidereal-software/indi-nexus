...
and is **never edited by hand**. A change that a user would notice lands with a file in
`changelog.d/` named `<slug>.<type>.md`, where the type is one of `added`, `changed`,
`fixed`, `removed`, `packaging` or `docs`:
...
One changelog covers all three packages, because they release together.
