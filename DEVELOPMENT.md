...
- **The path decides the package.** A commit touching `web/packages/react` lands in that
  package's changelog, not the Python one, so keep unrelated changes in separate commits
  when you want them filed separately.
