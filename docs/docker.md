...
starts with one. Leave `WEB_TOKEN` unset and it generates a random token instead, then
...
across restarts. Plain <http://localhost:8000/> loads the panel either way, but its
WebSocket handshake is refused with close code 1008 and no devices ever appear.

[The token](#the-token) covers the rest, including `WEB_ALLOW_ANONYMOUS`.
...
**Pin the patch tag for anything pointed at real hardware.** An observatory that pulls
`latest` gets a different version the day you cut a release, usually in the dark, usually
not on purpose.

`latest` is the newest *release*, not the newest commit, which is why the commands on this
page can safely leave the tag off.

`edge` is for tracking development or reproducing a bug report against a known commit, and
it is opt-in by name for that reason. It is `amd64` only: arm64 has to be emulated on a CI
runner, a release is the thing worth spending that on, and a Raspberry Pi wants a release
tag anyway. Release images are native on both, so an Apple Silicon Mac and an observatory
Pi each get a real build.
...
`WEB_TOKEN` in `compose.yaml` and set it to keep one URL across restarts.
...
it matters, because it is the one variable here that gives something away.

On its own it only **permits a non-loopback bind that has no token**, which `serve`
otherwise refuses. It never disables a token that is configured.

What serves the panel anonymously in this image is the entrypoint. When
`WEB_ALLOW_ANONYMOUS` is set and no token is, it starts `serve` with
`--allow-insecure-bind` and **no `--token`**, so there is no token to check. Set a token
...
Keep both by mounting a volume and pointing the variable into it:
...
Turn the log up when a driver is not appearing. That is the usual first move:
...
The container publishes its port, so the panel and `/ws` are reachable by anything that
can reach the host. `/ws` is the write surface: a frame sent there becomes an INDI `new*`
that moves hardware.

The bridge therefore always starts with a token, and the startup log prints the URL that
carries it:
...
WebSocket, which is the only place a browser can put one and the only route that accepts
`?token=`.

`/api` takes `Authorization: Bearer <token>` and nothing else, so `curl` and other
non-browser clients use the header there:
...
`network_mode: host` instead. That cannot be combined with `ports`, so delete the port
mapping when you add it. The container is then on the host's network and its ports are
already the host's.

Neither is on by default, because the first thing most people run here is a simulator.
...
the image serves the real panel and not the fallback debug page. The image runs as a
non-root user, and builds on `amd64` and `arm64`.
...
That runs `pytest tests/interop` against a real `indiserver` and real C++ drivers, which
otherwise happens only in the nightly CI job. Pass a command to narrow it:
